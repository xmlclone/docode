...
# group只是对父的参数分组，而不是click.group的概念，click.group对应的是下面的add_subparsers
g = p.add_argument_group('group')
g.add_argument('--foo')

# 要查看子命令的帮助，需要把必传参数填充后才可以查看
# python configargparse_demo0.py --genome 1 vcf sub1 --help
sub = p.add_subparsers(dest='sub1')
p1 = sub.add_parser("sub1")
p1.add_argument('-a')
p2 = sub.add_parser("sub2")
...
print(f"{options.foo=}")
# o
print(f"{options.sub1=}, {options.a=}")
